...
export function by(specs?: Parameters|null, order?: Order): Sorter {
  if (specs == null) {
    return { sort: ((entries: Entry[]): Entry[] => entries) };
  }
  ...
  return { sort: ((entries: Entry[]): Entry[] => entries.sort(callback)) };
}
...
function comparator(specs: Specifier[]): Comparator {
  ...
  return (left: Entry, right: Entry): number => {
    ...
    const evaluate = (result: number, [left, right]: [unknown, unknown]): number => (
      result === 0 ? compare(left, right) : result
    );
